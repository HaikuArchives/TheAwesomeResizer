...
void OptionWindow::MessageReceived(BMessage* message)
{
	switch(message->what)
	{
		...
		case SGB:
		case GRIP:
			((Resizer*)be_app)->fMainWin->PostMessage(message);break;
		...
		case COORD: ((Resizer*)be_app)->fMouseWin->PostMessage(message);break;
		case SAVEH:	fOptionView->fSavedHeight = atoi(fOptionView->fHeightTextbox->Text());break;
		case SAVEW:	fOptionView->fSavedWidth = atoi(fOptionView->fWidthTextbox->Text());break;
		case LOADH:	fOptionView->SetHeight(fOptionView->fSavedHeight);break;
		case LOADW:	fOptionView->SetWidth(fOptionView->fSavedWidth);break;
		case CHANGE_EFFECT:	fOptionView->ChangeEffect(message->FindInt16("Effect"));break;
		case APPLY:	fOptionView->ApplyEffect();break;
		...
		case UNLOCK_H: fLockH--;break;
		case UNLOCK_W: fLockW--;break;
